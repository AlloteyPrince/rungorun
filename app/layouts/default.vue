  ...
  <div class="min-h-screen bg-runblack text-white font-['Urbanist']">
    ...
    <nav class="fixed top-0 w-full z-[100] glass py-4 md:py-6">
      <div class="container mx-auto px-5 sm:px-6 max-w-6xl flex justify-between items-center">
        ...
        <!-- Logo -->
        <div class="text-xl md:text-2xl font-black italic tracking-tighter uppercase font-['Poppins']">
          RUN<span class="text-rungreen neon-glow">GO</span>RUN
        </div>

        <!-- Desktop Links (Font increased to text-xs) -->
        ...
        <div class="flex items-center gap-6">
          <!-- Accra Live Pulse (Slightly larger text for balance) -->
        ...
        <div v-if="isMobileMenuOpen" class="fixed inset-0 top-[72px] bg-black/95 backdrop-blur-2xl z-50 md:hidden overflow-hidden">
          <div class="flex flex-col p-10 gap-8 h-full justify-center">
            <button 
              ...
              :key="link.name" 
              @click="handleNavClick(link.path)"
              class="text-5xl font-black italic uppercase tracking-tighter text-left font-['Poppins'] hover:text-rungreen transition-colors"
            >
              {{ link.name }}
            </button>
            <button 
