...
<template>
  <div class="bg-runblack text-white selection:bg-rungreen selection:text-black min-h-screen font-['Urbanist',_sans-serif]">
    ...
    <!-- ── HERO ── -->
    <section id="hero" class="relative h-screen flex items-center overflow-hidden bg-runblack">
      ...
      <div class="container mx-auto px-5 sm:px-6 max-w-6xl relative z-20">
        <div :class="['transition-all duration-1000 transform', visibleSections.has('hero') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20']">
          ...
          <button @click="scrollToSection('#available')" class="group px-8 sm:px-14 py-4 sm:py-6 bg-white/5 border border-white/20 rounded-3xl hover:bg-rungreen hover:text-black transition-all duration-500">
            <span class="font-black uppercase italic text-xs sm:text-sm tracking-widest">EXPLORE PRODUCTS</span>
          </button>
        </div>
      </div>
    </section>
    ...
    <div class="container mx-auto px-5 sm:px-6 max-w-6xl">
      ...
      <section id="science" class="py-24 sm:py-44 scroll-mt-32">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-10">
          ...
          ]" :key="i" class="angle-glow-card p-8 sm:p-12 rounded-[40px] md:rounded-[60px] border border-white/10 relative overflow-hidden group text-left">
      ...
      <section id="available" class="py-24 sm:py-44 scroll-mt-32">
        ...
        <div class="flex justify-center md:justify-start">
          <NuxtLink to="/categories" class="group flex items-center gap-6 px-10 py-5 bg-rungreen/5 border border-rungreen/20 rounded-full hover:bg-rungreen hover:text-black transition-all duration-500">
            <span class="font-black uppercase italic text-xs tracking-[0.3em]">Browse Full Inventory</span>
            <Icon name="ph:caret-double-right-bold" class="text-xl group-hover:translate-x-2 transition-transform" />
          </NuxtLink>
        </div>
      </section>
      ...
      <section id="consultation" class="py-24 sm:py-44 scroll-mt-32">
        <div class="angle-glow-card rounded-[50px] md:rounded-[80px] p-8 md:p-24 border border-white/10 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center overflow-hidden relative text-left">
...
.angle-glow-card {
  ...
  background-image: 
    radial-gradient(circle at top left, rgba(223, 255, 0, 0.12) 0%, transparent 40%),
    radial-gradient(circle at bottom right, rgba(223, 255, 0, 0.12) 0%, transparent 40%);
}
