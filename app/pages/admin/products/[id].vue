...
const deleteProduct = async () => {
  if (!confirm(`Delete "${product.value.name}" permanently? This cannot be undone.`)) return;
  const { error: err } = await supabase.from("products").delete().eq("id", product.value.id);
  if (!err) router.push("/admin/products");
};
  ...
  <div class="min-h-screen bg-runblack font-['Urbanist',_sans-serif]">
    <header class="sticky top-0 z-50 bg-runblack/80 backdrop-blur border-b border-white/5 py-4 px-5 sm:px-8">
      <div class="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-3">
          <button
            @click="deleteProduct"
            class="px-5 py-2 border border-red-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest text-red-400/60 hover:text-red-400 hover:border-red-500/40 transition-all"
          >
            Delete
          </button>
          <NuxtLink
    ...
    <div class="max-w-4xl mx-auto px-5 sm:px-8 py-10">
      ...
      <div v-else-if="notFound" class="text-center py-20">
        <p class="text-gray-400 font-bold italic text-xl mb-4">Product not found.</p>
        <NuxtLink to="/admin/products" class="text-rungreen hover:underline font-bold">
          ← Back to Products
        </NuxtLink>
      </div>
      ...
      <template v-else>
        ...
        <AdminProductForm v-model="product" :id-editable="false" />
      </template>
    </div>
  </div>
</template>
