...
<template>
  <div
    class="bg-runblack text-white selection:bg-rungreen selection:text-black font-['Urbanist',_sans-serif]"
  >
      ...
      <div class="container mx-auto px-5 sm:px-6 max-w-7xl w-full">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
          ...
          <div class="lg:col-span-7 flex flex-col gap-3">
            ...
            <div
              class="glass-card-deep rounded-[32px] overflow-hidden border border-white/5 flex items-center justify-center relative group main-image-card"
            >
              <div
                class="absolute top-5 left-5 bg-rungreen text-black text-[10px] font-black px-3 py-1.5 uppercase rounded-md z-10 italic"
              >
              ...
              <div
                v-else
                class="flex flex-col items-center justify-center gap-3 opacity-20"
              >
                <Icon
                  name="ph:sneaker-move-bold"
                  class="text-white text-[8rem]"
                />
                <span
              ...
              <div
                class="absolute inset-0 bg-gradient-to-t from-runblack/80 to-transparent opacity-60"
              ></div>
            </div>
            ...
            <div class="grid grid-cols-4 gap-3">
              <div
                ...
                @click="
                  product.images[i - 1]
                    ? (activeImage = product.images[i - 1])
                    : null
                "
                :class="[
          ...
          <div class="lg:col-span-5 flex flex-col gap-4 text-left">
            <header class="space-y-1.5">
              <div
                class="flex items-center gap-4 text-rungreen font-black italic tracking-[0.3em] text-xs uppercase"
              >
                <span
                  class="w-1.5 h-1.5 bg-rungreen rounded-full animate-pulse"
                ></span>
                <span>{{ product.category }}</span>
              </div>
              <h1
                class="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-[0.9] font-['Poppins']"
              >
                {{ product.name }}
              </h1>
              <div
                class="text-2xl font-black italic text-rungreen tracking-tighter flex items-center gap-3"
              >
                <span
                  class="text-sm opacity-50 not-italic uppercase tracking-widest font-bold"
                  >MSRP</span
                >
                GHS {{ product.price }}
              </div>
            ...
            <p
              class="text-gray-300 font-bold italic text-base leading-relaxed border-l-4 border-rungreen pl-4"
            >
              {{ product.description }}
            </p>
            ...
            <div v-if="product.sizing === 'sized'" class="space-y-2">
              <h3
                class="text-white font-black uppercase text-xs tracking-[0.4em] italic opacity-60"
              >
                Select_Size
              </h3>
              <div class="flex flex-wrap gap-2">
                <button
                  ...
                  :class="[
                    ...
                    selectedSize === size
                      ? 'bg-rungreen border-rungreen text-black scale-105'
            ...
            <div class="space-y-2">
              ...
              <div
                class="flex items-center gap-5 bg-white/5 w-fit px-5 py-2 rounded-xl border border-white/10"
              >
                <button
                  @click="updateQuantity(-1)"
                  class="text-xl font-black hover:text-rungreen transition-colors px-1"
                >
                  -
                </button>
                <span
                  class="text-xl font-black w-10 text-center text-rungreen font-['Poppins']"
                  >{{ quantity }}</span
                >
                <button
                  @click="updateQuantity(1)"
            ...
            <!-- TECHNICAL LOG -->
            <div
              class="glass-card-deep p-4 rounded-[24px] border border-white/5 space-y-2"
            >
              <h3
                class="text-rungreen font-black uppercase text-xs tracking-[0.4em] italic mb-2"
              >
                Technical_Log
              </h3>
              <div
              ...
              >
                ...
                >
                <span
                  class="text-sm font-black uppercase italic tracking-tighter"
                  >{{ spec.value }}</span
                >
              </div>
            </div>
            ...
            >
              DEPLOY ORDER
              <Icon
                name="ph:whatsapp-logo-bold"
                class="text-3xl group-hover:scale-110 transition-transform"
              />
            </button>

            <p
              class="text-xs font-black uppercase tracking-[0.2em] text-center opacity-40 italic"
            >
              *All orders are processed via the secure Run Go Run WhatsApp
              Terminal
            </p>
          </div>
        </div>
